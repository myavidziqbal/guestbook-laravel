...
<body>
  ...
  <h1 class="text-center mb-4 mt-5">Edit Work Order</h1>
  ...
  <div class="container mb-5">
      <div class="row justify-content-center">
          <div class="col-8">
            <div class="card">
              <div class="card-body">
                <form action="/updatewo/{{$data->id}}" method="POST" enctype="multipart/form-data">
                  @csrf
                  <div class="mb-3">
                    <label for="exampleInputEmail1" class="form-label">Peminta Work Order</label>
                    <input type="text" name="name_req" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" value="{{$data->name_req}}">
                    @error('nosurat')
                  ...
                  <div class="mb-3">
                    ...
                    <input type="text" name="name_receipt" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" value="{{$data->name_receipt}}">
                    @error('nama')
                      <div class="alert alert-danger">{{ $message }}</div>
                    @enderror
                  </div>
                  <div class="mb-3">
                    ...
                    <select class="form-select" name="dept" aria-label="Default select example">
                      <option selected>{{$data->dept}}</option>
                  ...
                  <div class="mb-3">
                    ...
                    <input type="text" name="issue" class="form-control" id="exampleFormControlTextarea1" rows="3" value="{{$data->issue}}">
                    @error('nama')
                      <div class="alert alert-danger">{{ $message }}</div>
                    @enderror
                  </div>
                  <div class="mb-3">
                    ...
                    <input type="text" name="action" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" value="{{$data->action}}">
                    @error('nama')
                      <div class="alert alert-danger">{{ $message }}</div>
                    @enderror
                  </div>
